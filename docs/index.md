...
- iPadの2カラム表示で、スレッドの既読位置が記憶されない
  - 改善予定です。回避策として、「スレッドメニュー」を開くたび、またはAppスイッチャーを一瞬表示するたびに既読位置が記憶されます。
    - 別の回避策として、Split ViewやSlide Overで1カラムで使用すると、iPhoneと同じ動作になります。
  - iPadの2カラム画面は考慮する事が多く、またどうしてもiPhoneでの動作確認が優先となってしまうため、特にバグが多いです。少しずつ修正しますので何卒ご了承ください。
- 全体的に動作がもっさりしている
